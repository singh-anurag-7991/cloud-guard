...
- 2026-08-01 (same session, AWS onboarding audit + CGO removal): Reviewed the "1-click CloudFormation" onboarding flow end-to-end and found **4 real bugs, all of which would have blocked any real customer**: (1) `deployments/cloudformation.yaml` had `SaaSAccountID` defaulted to the placeholder `123456789012`, so the generated trust policy pointed at a non-existent account; now `143506099819`. (2) The CFN trust policy enforces an `sts:ExternalId` condition but `internal/aws/client.go` never sent one — every AssumeRole would have failed with AccessDenied and no scan could ever run. Added `ExternalID()` (env-overridable via `CLOUDGUARD_EXTERNAL_ID`) and wired it into `stscreds.NewAssumeRoleProvider`. (3) `GET /cloudformation.yaml` 404'd in production because the Dockerfile only copied the binary and `web/`, not `deployments/` — so the "Download Template" button was dead. (4) The "1-Click AWS Launch" button pointed at a bare CloudFormation create page with no `templateURL`, i.e. it was never actually 1-click; `handleCloudFormationURL` also still had `http://localhost:8080/...` hardcoded. Rewrote as a proper quick-create link builder gated on `CF_TEMPLATE_S3_URL`, and made the dashboard copy an honest numbered 4-step flow. **Note: AWS only accepts an S3-hosted `templateURL`, so true 1-click still requires uploading the template to a public S3 bucket and setting that env var — until then it is a 2-step flow.** The landing page still advertises "1-click CloudFormation"; that claim is not fully delivered yet and should be settled before marketing/outreach.
  - Verified live after deploy: `/cloudformation.yaml` returns 200 with `SaaSAccountID=143506099819` and `ExternalId=cloud-guard-saas`, `/healthz` OK.
  - **Replaced `mattn/go-sqlite3` with pure-Go `modernc.org/sqlite`.** The CGO build had blocked progress three separate times (OOM kill, then repeated 10-12 min builds, then an SSH `connection reset by peer` mid-build once the app + Caddy containers were also competing for RAM — which silently killed the build since it was a child of the SSH session). Dockerfile now uses `CGO_ENABLED=0`, drops gcc/musl-dev and sqlite-libs, and no longer needs `-p 1`. Build went from ~10-12 min to ~1-2 min. Also added `busy_timeout(10000)` + `journal_mode(WAL)` to the DSN, which fixes a latent "database is locked" risk since the background scheduler and the web handlers both write to the same SQLite file.
  - Deploy lesson: run long builds detached (`systemd-run --unit=... --collect`) rather than as a direct child of the SSH session, otherwise a dropped connection kills the build silently.
- 2026-08-01 (same session, full audit + auth build + end-to-end verification): Anurag asked for every problem to be fixed and every feature verified on the server, then a report. Full audit found **11 bugs, 3 of them critical**. See `coach/STATUS-REPORT.md` for the complete write-up.
  - **Critical #1 — there was no authentication at all.** An empty `CLERK_SECRET_KEY` silently granted every request tenant `"default"`, leaving the dashboard, `/connect`, `/scan` and all `/api/*` routes open to the internet (verified: `/dashboard` returned 200 with no credentials). Anurag independently spotted this mid-session. Built real signup/login: bcrypt passwords, opaque server-side session tokens, HttpOnly+Secure+SameSite cookies, 7-day TTL, `/login` `/signup` `/logout` routes and pages.
  - **Critical #2 — multi-tenancy was fake.** `parseTokenClaims` never verified the JWT and always returned `"default"`; any random string passed as a token and all users shared one tenant, so any customer would have seen every other customer's findings. Each signup now gets an isolated `tenant_id` (verified by test: second user saw zero of the first user's data).
  - **Critical #3 — the database never persisted.** `DB_PATH` defaulted to `/root/cloudguard.db` while the Docker volume mounts at `/root/data`, so every redeploy silently wiped all accounts and findings. Fixed, plus WAL + busy_timeout to remove a latent "database is locked" risk between the scheduler and web handlers.
  - Also fixed: missing `sts:ExternalId` (would have broken AssumeRole for *every* customer), placeholder SaaS account ID, `/cloudformation.yaml` 404, silent scan failures reported as success, dead footer links, and the CGO build that OOM-killed the box three times (swapped to pure-Go `modernc.org/sqlite`; build 10-12 min → 1-2 min).
  - **AWS infra completed via console** (the `cloudguard-deploy` IAM user lacks IAM and `iam:PassRole` permissions, so CLI couldn't do it): created `CloudGuardAppRole` + instance profile with `sts:AssumeRole` scoped to `CloudGuardReadOnlyRole-*` only, attached it to the instance, and created a test customer role with the ExternalId trust condition to prove the path.
  - **Verified live: 24/24 end-to-end tests pass** — auth, session lifecycle, tenant isolation, ARN validation, connect, and a real scan completing in 3.8s with no AccessDenied. Container logs confirm EC2/S3/RDS scanners all ran clean and the background hourly scheduler fires on its own. "0 findings" is correct: this account has nothing to flag.
  - **Three things still block selling:** (1) Cost Explorer is not enabled on the account (`User not enabled for cost explorer access`) — a headline advertised feature that silently returns nothing, and every customer will need to enable it during onboarding; (2) Stripe is still on test keys so nobody can pay; (3) the landing page overclaims — "1-click CloudFormation" is really 2-step without an S3-hosted template, homepage stats (847 resources, $620/mo, 99.9% SLA) are hardcoded placeholders shown as real, and Data Guard / Shield are marketed with no code behind them. For a security product these are unusually costly overclaims because a technical buyer can disprove them in one click.
